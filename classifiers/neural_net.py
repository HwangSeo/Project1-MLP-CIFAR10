...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # 순전파: 은닉층 - ReLU - 출력층 (scores)
    hidden = np.maximum(0, X.dot(W1) + b1)  # ReLU 적용
    ...
    # 소프트맥스 손실 계산
    scores -= np.max(scores, axis=1, keepdims=True)  # 숫자 안정화
  ...
  def predict(self, X):
    ...
    # 순전파 계산 후 가장 큰 점수의 인덱스를 예측값으로 반환
